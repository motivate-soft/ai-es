...
function main() {
  ...
  var layers = getLayersBelow(isInclActive);
  selectAllOnLayers(layers);
}

/**
 * Get all layers below the active layer
 * @param {boolean} isInclActive - include the active layer
 * @return {array} out - layers
 */
function getLayersBelow(isInclOrig) {
  ...
  if (isInclOrig && isAvailable(aLayer)) out.push(aLayer);
